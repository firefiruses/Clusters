...
class _LazyUmapModule:
    def __getattr__(self, name):
        import importlib
        module = importlib.import_module("umap")
        globals()["umap"] = module
...
class HighDimVisualizer:
    ...
    def prepare_dataset(self, data, target=None, feature_names=None, scale=True,
                        max_samples=None):
        """
        Convert a sklearn Bunch, pandas DataFrame, or array-like object to
        X, labels, and feature names for clustering/visualization.
        """
        import pandas as pd

        y = None

        if hasattr(data, "data") and not isinstance(data, (pd.DataFrame, np.ndarray)):
            X = np.asarray(data.data)
            y = getattr(data, "target", None)
    ...
    def cluster_dataset(self, X, method="kmeans", n_clusters=10, **kwargs):
        """Run clustering and return predicted cluster labels."""
        method = method.lower()
        ...
        if method == "kmeans":
            ...
            raise ValueError("Supported methods: 'kmeans', 'dbscan'")

        labels = model.fit_predict(X)
        return labels, model

    def plot_cluster_projection(self, X, labels, title="Проекция кластеров",
                                target=None):
        """Plot a quick 2D PCA projection colored by cluster labels."""
        pca = PCA(n_components=2, random_state=self.random_state)
        X_pca = pca.fit_transform(X)

        fig, axes = plt.subplots(1, 2 if target is not None else 1,
                                 figsize=(14 if target is not None else 7, 5))
        if target is None:
            axes = [axes]

        axes[0].scatter(X_pca[:, 0], X_pca[:, 1], c=labels, cmap="tab20",
        ...
        if target is not None:
            axes[1].scatter(X_pca[:, 0], X_pca[:, 1], c=target, cmap="tab10",
                            s=24, alpha=0.75, edgecolors="none")
    ...
    def plot_interactive_3d_clusters(self, X, labels, output_file="interactive_3d_plot.html",
                                     target=None, title="3D-визуализация кластеризации"):
        """Build and save an interactive 3D PCA Plotly figure."""
        pca = PCA(n_components=3, random_state=self.random_state)
        X_pca = pca.fit_transform(X)

        hover = [f"кластер={cluster}" for cluster in labels]
        if target is not None:
            hover = [
                f"кластер={cluster}<br>истинная_метка={true_label}"
                for cluster, true_label in zip(labels, target)
            ]
        ...
        fig = go.Figure(data=[
            go.Scatter3d(
                ...
                z=X_pca[:, 2],
                mode="markers",
                marker=dict(size=4, color=labels, colorscale="Viridis",
                            opacity=0.82, showscale=True),
                text=hover,
                hoverinfo="text",
            )
        ])
        ...
        fig.update_layout(
            title=f"{title}<br>Доля дисперсии PCA: {variance.sum():.1%}",
            scene=dict(
    ...
    def run_clustering_visualization(self, data, target=None, feature_names=None,
                                     method="kmeans", n_clusters=10, scale=True,
                                     max_samples=1200, output_file="interactive_3d_plot.html",
                                     **cluster_kwargs):
        """
        Full pipeline for an arbitrary dataset: prepare data, cluster it, and
        draw static 2D plus interactive 3D PCA visualizations.
        """
        X, true_labels, names = self.prepare_dataset(
            data,
            target=target,
        ...
        cluster_labels, model = self.cluster_dataset(
            X,
            method=method,
            n_clusters=n_clusters,
            **cluster_kwargs,
        ...
        self.plot_cluster_projection(
            X,
            cluster_labels,
            title=f"Кластеры {method.upper()}",
            target=true_labels,
        )
        self.plot_interactive_3d_clusters(
            X,
            cluster_labels,
    ...
    def prepare_dataset(self, data, target=None, feature_names=None, scale=True,
                        max_samples=None, drop_columns=None):
        import pandas as pd
    ...
    def cluster_dataset(self, X, method="kmeans", n_clusters=10, **kwargs):
        ...
        if method == "kmeans":
            ...
            raise ValueError("Supported methods: 'kmeans', 'dbscan', 'agglomerative'")
    ...
    def evaluate_cluster_labels(self, X, labels, target=None):
        import pandas as pd

        unique_labels = set(labels)
        score_labels = [label for label in unique_labels if label != -1]
        result = {
            "clusters": len(score_labels),
        ...
        if target is not None:
            result["adjusted_rand"] = adjusted_rand_score(target, labels)
    ...
    def reduce_dimensions(self, X, method="pca", n_components=2, **kwargs):
        ...
        if method == "pca":
            ...
            reducer = umap.UMAP(n_components=n_components, random_state=self.random_state, **kwargs)
        else:
            raise ValueError("Supported methods: 'pca', 'umap'")
        embedding = reducer.fit_transform(X)
        return embedding, reducer

    def plot_cluster_projection(self, X, labels, title="Cluster projection",
                                target=None):
        pca = PCA(n_components=2, random_state=self.random_state)
        X_pca = pca.fit_transform(X)
        ...
        if target is not None:
            target_codes, target_names = self.encode_labels(target)
            axes[1].scatter(X_pca[:, 0], X_pca[:, 1], c=target_codes, cmap="tab10",
                            s=24, alpha=0.75, edgecolors="none")
            axes[1].set_title("Known labels")
            axes[1].set_xlabel(f"PC1 ({pca.explained_variance_ratio_[0]:.1%})")
            axes[1].set_ylabel(f"PC2 ({pca.explained_variance_ratio_[1]:.1%})")
            axes[1].grid(True, alpha=0.25)
            handles, _ = axes[1].collections[0].legend_elements()
            axes[1].legend(handles, [str(name) for name in target_names], title="label")
    ...
    def plot_interactive_3d_clusters(self, X, labels, output_file="interactive_3d_plot.html",
                                     target=None, title="3D cluster visualization"):
        pca = PCA(n_components=3, random_state=self.random_state)
        X_pca = pca.fit_transform(X)

        hover = [f"cluster={cluster}" for cluster in labels]
        if target is not None:
            hover = [
                f"cluster={cluster}<br>label={true_label}"
                for cluster, true_label in zip(labels, target)
            ]
        ...
        variance = pca.explained_variance_ratio_
        fig.update_layout(
            title=f"{title}<br>PCA explained variance: {variance.sum():.1%}",
            scene=dict(
                xaxis_title=f"PC1 ({variance[0]:.1%})",
                yaxis_title=f"PC2 ({variance[1]:.1%})",
                zaxis_title=f"PC3 ({variance[2]:.1%})",
    ...
    def run_clustering_visualization(self, data, target=None, feature_names=None,
                                     method="kmeans", n_clusters=10, scale=True,
                                     max_samples=1200, output_file="interactive_3d_plot.html",
                                     drop_columns=None, **cluster_kwargs):
        X, true_labels, names = self.prepare_dataset(
            data,
            target=target,
            feature_names=feature_names,
        ...
        metrics = self.evaluate_cluster_labels(X, cluster_labels, true_labels)
        print(metrics)
        self.plot_cluster_projection(
            X,
            cluster_labels,
            title=f"{method.upper()} clusters",
            target=true_labels,
        )
        self.plot_interactive_3d_clusters(
            X,
